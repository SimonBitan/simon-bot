...
def run_keepalive():
    # Replit provides PORT; fall back to 3000 when running locally
    port = int(os.getenv("PORT", "3000"))
    app.run(host="0.0.0.0", port=port)

# Start the keepalive server in the background
...
BIRTHDAYS: dict[str, str] = {}  # {user_id: "MM-DD"}
    ...
    try:
        with open(CSV_PATH, newline="") as f:
            ...
            for row in reader:
                ...
                if uid and md:
                    BIRTHDAYS[uid] = md
        print(f"[Startup] Loaded {len(BIRTHDAYS)} birthdays from {CSV_PATH}")
    except FileNotFoundError:
        print(f"[Startup] WARN: {CSV_PATH} not found; no birthdays loaded")

load_birthdays_from_csv()

# --- helpers ---
def is_cmd(content: str, name: str) -> bool:
    """Match '!cmd' or '!cmd ...' case-insensitively with extra whitespace tolerance."""
...
async def run_check_birthdays_once(target_channel: discord.abc.Messageable | None = None):
    ...
    try:
        ...
        print(f"Error in run_check_birthdays_once: {e}")
        if target_channel is not None:
            await target_channel.send(f"⚠️ Error checking birthdays: `{e}`. Check logs.")
...
async def on_ready():
    ...
    # Choose your daily send time (UTC). Adjust hour/minute as desired.
